...
export default {
  ...
  data() {
    return {
      ...
      menu: [
        ...
        //   disabled: ({ currentRow }) => currentRow.id == null,
        //   on: currentRow => {
        //     this.publicConfirm(1, currentRow);
        //   }
        // },
        // {
        //   label: "取消发布",
        ...
        //   disabled: ({ currentRow }) => currentRow.id == null,
        //   on: currentRow => {
        //     this.publicConfirm(2, currentRow);
        //   }
        // },
        {
          label: "删除",
      ...
      column: [
        ...
        { prop: "quantity", label: "答卷次数", align: "center" },
          ...
          formatter: row => {
            return (
              row.publishStatus=='已发布'?
                <el-button type="text" onClick={this.publicConfirm.bind(this,2, row)}>
                  取消发布
                </el-button>:
                <el-button type="text" onClick={this.publicConfirm.bind(this,1, row)}>
                  发布
                </el-button>
            )
          }
  ...
  methods: {
    insert() {
      this.$router.push("./release?paperType=tool");
    },
    update(count, row) {
      this.updateId = row.id;
      if (count === 1) {
        this.$router.push(`./release?paperType=tool&id=${row.id}&copy=true`);
      } else {
        this.$router.push(
          `./release?paperType=tool&id=${row.id}&copy=true`
        );
      }
    },
    publicConfirm(count,row){
      console.log(count,row)
      let text =
        count === 1? "是否发布问卷？":"是否取消发布？";
      this.$confirm(text, "提示", {
        ...
        .then(() => {
          if (count === 1) this.onpublic(row.id);
          else this.cancelPublish(row.id);
        })
        .catch(() => {});
    },
    onpublic(id){
      this.$http.paperPublish({ id }).then(res => {
        console.log(res);
    ...
    // 取消发布
    cancelPublish(id){
      this.$http.cancelPublish({ id }).then(res => {
        console.log(res);
        this.$refs.table.onSearchKeep();
        this.$message({
          message: "已取消",
